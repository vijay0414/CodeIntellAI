        ...
        <div className="rounded-2xl border border-[#1e2333] bg-[#11151f] p-4">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3">
            <label className="text-xs font-semibold uppercase tracking-[0.25em] text-slate-500">
              Question count
              <select value={count} onChange={(e) => onCountChange(Number(e.target.value))} className="ml-2 rounded-lg border border-[#1e2333] bg-[#080b11] px-3 py-2 text-sm text-slate-200">
            ...
            <button onClick={onLoad} className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-blue-500">
              Generate questions
            </button>
          </div>
        </div>
  ...
  return (
    <div className="space-y-3 panel-enter">
      {result.questions.map((q, index) => (
        <div key={index} className="rounded-2xl border border-[#1e2333] bg-[#11151f] p-4">
          <div className="flex flex-wrap items-center gap-2">
            <span className={`badge ${q.difficulty === 'hard' ? 'bg-red-500/15 text-red-400 border border-red-500/25' : q.difficulty === 'medium' ? 'bg-amber-500/15 text-amber-400 border border-amber-500/25' : 'bg-green-500/15 text-green-400 border border-green-500/25'}`}>
              {q.difficulty}
            </span>
