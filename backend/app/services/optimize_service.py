"""
Optimize service — LLM-powered code optimization with genuine improvement gate.
"""
...
from app.utils.prompt_templates import optimize_system_prompt, optimize_user_prompt
...
async def run_optimize(
    ...
    user_prompt = optimize_user_prompt(code, language)

    raw = await llm.chat(system_prompt, user_prompt)
    logger.debug("Optimize LLM raw response (first 200): %.200s", raw)

    try:
        data = parse_llm_json(raw)
        return OptimizeResponse.model_validate(data)
    except (LLMJsonParseError, ValidationError, TypeError) as exc:
        logger.warning("Optimize parse failed on first attempt: %s", exc)

    logger.info("Retrying optimize LLM call with correction prompt.")
    try:
        ...
        logger.error("Optimize parse failed after retry: %s", exc)
        raise ValueError(
